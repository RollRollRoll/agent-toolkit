---
name: triage
description: 按 triage 状态机处理 issue 和外部 PR：分类、验证、补齐必要信息，并产出 agent 可以直接执行的 brief。
...
状态角色发生冲突时，先指出冲突并询问维护者，在得到结论前暂停后续处理。
...
4. **补齐信息（如果需要）。** 如果请求还不够完整，调用 `grilling` 和 `domain-modeling`，
   按轮次追问并同步梳理领域术语，
...
**先确认你即将做什么**（改哪些角色、发什么评论、关不关），然后动手。**跳过深入追问。**
如果是在没有深入追问过的情况下挪到 `ready-for-agent`，**问一句他要不要写一份 agent brief**。
...
将追问中已经确认的内容全部记录到 `established so far`，避免后续重复确认。
