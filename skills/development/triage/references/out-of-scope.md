...
仓库中的 `.out-of-scope/` 目录用于长期记录被拒绝的功能请求，主要有两个用途：

1. **组织记忆**：保留功能被拒绝的原因，不随 issue 关闭而丢失。
2. **去重**：新 issue 与既有记录重复时，可以引用原决定，无需重新讨论同一问题。
...
每个概念使用一个文件，而不是每个 issue 一个文件。指向同一需求的多个 issue 归入同一记录。
