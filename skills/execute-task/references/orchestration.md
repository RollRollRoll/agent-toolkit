...
> 任务内只派执行 subagent（TDD 闭环见 [execution-loop.md](execution-loop.md)），review / fix 只在阶段 3。
...
- **每任务只派一类 subagent（执行）**：fresh 上下文，只带这一个任务需要的信息；
  **主 agent 做编排**：确认阶段 0 提交授权 → 记录干净基线 → 派执行 → 按闸门一核证据 →
  checkbox 与代码一起 commit → ignored ledger → clean 校验 → 下一个；不自己埋头糊代码。
  任务内**没有审查轮与修复轮**——review / fix 派发只发生在阶段 3 整体验收。
  派发 prompt 照抄 [handoff-templates.md](handoff-templates.md)。
- **文件 handoff，不粘正文**：大块产物写成文件、派发只传路径——粘进派发 prompt 的内容会常驻主 agent
  ...
  - **执行报告** `task-N-report.md`：执行 subagent 写详细报告——实现了什么、**每个行为的红 → 绿证据**
    （红的命令与失败输出 + 为何预期失败、绿的命令与通过输出）、改动文件、问题与疑虑；闸门一读它核证据；
...
- **派发 prompt 只描述这一个任务**：一行任务定位（在整个项目里的位置）+ 简报路径（"先读它，它是你的需求"）+
  约定的 seam + 前序任务已定的接口 / 决策（简报无法知道的）+ 报告文件路径与回执契约。
  **不要**把前面任务的累积摘要粘进后面任务的派发。
- **执行回执 = 状态 + 短摘要**（细节都在报告文件里，回执控制在十行内）：
  状态 + commit 无（不 commit）+ 一行测试摘要 + 疑虑 + 报告路径。四种状态、各有处理：
  - `DONE` → 按闸门一核三样证据（红 → 绿证据齐、主 agent 自己复跑验证命令绿、HEAD 未漂移且范围没越界）→ commit；
  - `DONE_WITH_CONCERNS` → 先读疑虑：涉及正确性 / 范围的先处理再过闸门；结构观察项（如"这文件越来越大"）
    记进账本留给阶段 3 的 architecture 轴，继续往下；
  - `NEEDS_CONTEXT` → 补上缺的上下文**重派**；
  - `BLOCKED` → 升级阶梯：补上下文重派 → 换更强模型（平台可选时）→ 拆小任务 → 退回上游 / 交用户——
    **不许什么都不变原样重试**，也不许无视升级硬冲。
- **证据不齐怎么办**：报告只有绿、没有红，或验证命令没实跑 → 退回同一执行 subagent 补齐（它有上下文）；
  补不出（说明没真的先红）→ 记录在案并告知用户，**别默认它做了 TDD**、也别自己替它补一段事后测试当证据。
...
- **平台**：派发前读取 [platform-agents.md](platform-agents.md)。Claude Code 保留 Task / Agent 工具路径；Codex 使用 subagent agent thread，并按角色选内置或自定义 agent。**subagent 不可用的环境**（无该能力）→
  降级为主 agent **顺序执行**同一套闭环纪律（红 → 绿 TDD → 验证 → 证据核对 → commit），不卡死。
...
- 每任务派了 fresh 执行 subagent（或降级顺序执行）？主 agent 在编排而非糊代码？任务内没有多派 review / fix？
- handoff 走的是文件（简报 / 报告传路径）而非粘贴正文？简报是 `task-brief.sh` 抽取的（没手抄）、含约定的 seam？
  执行回执带了状态（DONE / BLOCKED…）且按状态处理了？
- 过闸门一核了三样证据（红 → 绿齐、主 agent 复跑验证命令绿、HEAD 未漂移）？证据不齐时退回补齐而非默认它做了 TDD？
