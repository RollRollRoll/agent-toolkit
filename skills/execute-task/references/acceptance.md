...
任务由**主 agent 自己**执行，任务内**不派任何 subagent，也没有独立审查轮与修复轮**——
质量由红 → 绿 TDD 本身承担，闸门一只核**可机械判定的证据**
（细则见 [execution-loop.md](execution-loop.md) 第七节）：

- **验收标准逐条对上** split-task 给的那份，不多不少。
- **红 → 绿证据齐**：执行记录里每个行为都有实现前的失败输出（含"为何预期失败"）+ 实现后的通过输出，
  且是当场写的。只有绿没有红 → 把那个行为重做一遍红 → 绿；做不到就记录并告知用户，
  别拿事后补的测试冒充红证据。
...
- 这是任务级"做完没做完"的客观判据，不靠"看起来写好了"，也不靠再派一个 agent 说"看过了"。
  自己做的活自己核也成立——判据是**落盘的输出**，不是"我记得都跑绿了"。
- 执行期看到的结构问题记进执行记录与账本，**留给闸门二的 architecture 轴**，不在任务内开审查轮。
...
派发时把各任务执行记录里记下的结构疑虑一并交给它。审出的 Critical / Important **派独立 fix subagent**
