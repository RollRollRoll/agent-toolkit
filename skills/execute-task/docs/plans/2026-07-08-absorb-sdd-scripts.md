...
- skill 脚本目录：`$REPO/skills/execute-task/scripts`（下称 `$SCRIPTS`）。
- 本计划内的 commit 逐任务做（已获用户批准）；**不 push**——发布 push 由用户在收尾环节拍板。
- 环境变量约定（每个命令块可直接复制）：

```bash
export REPO=/root/workspace/agent-toolkit
export SCRIPTS=$REPO/skills/execute-task/scripts
...
**Files:**
- Create: `skills/execute-task/scripts/workspace.sh`
...
```bash
cd "$REPO"
git add skills/execute-task/scripts/workspace.sh
...
**Files:**
- Modify: `skills/execute-task/scripts/review-diff.sh`
...
- [x] **Step 1: 构造 U10 可检验 fixture 并证明现状不满足（红）**
...
```bash
cd "$REPO"
git add skills/execute-task/scripts/review-diff.sh
...
**Files:**
- Create: `skills/execute-task/scripts/task-brief.sh`
...
```bash
cd "$REPO"
git add skills/execute-task/scripts/task-brief.sh
...
**Files:**
- Create: `skills/execute-task/scripts/acceptance-diff.sh`
...
- [x] **Step 1: 准备多 commit 场景并证明脚本不存在（红）**
...
```bash
cd "$REPO"
git add skills/execute-task/scripts/acceptance-diff.sh
...
**Files:**
- Modify: `skills/execute-task/references/handoff-templates.md`
- Modify: `skills/execute-task/references/orchestration.md`
- Modify: `skills/execute-task/references/acceptance.md`
...
- [x] **Step 1: handoff-templates.md 第〇节——简报步骤脚本化**
...
- [x] **Step 2: handoff-templates.md 第〇节步骤 4——diff 描述补 -U10**
...
- [x] **Step 3: handoff-templates.md 新增第四节模板，原第四节顺延为第五节**
...
- [x] **Step 4: handoff-templates.md 自查清单补两条**
...
- [x] **Step 5: orchestration.md 第四节——简报 bullet 与 diff bullet**
...
- [x] **Step 6: orchestration.md 第五节——ledger 记起点 commit**
...
- [x] **Step 7: acceptance.md 第二节——输入准备与 fix 循环**
...
- [x] **Step 8: acceptance.md 收尾自检补一条**
...
grep -n 'task-brief.sh\|acceptance-diff.sh\|workspace.sh\|review-diff.sh' \
  skills/execute-task/references/handoff-templates.md \
  skills/execute-task/references/orchestration.md \
  skills/execute-task/references/acceptance.md
```
...
```bash
cd "$REPO"
git add skills/execute-task/references/
...
**Files:**
- Modify: `skills/execute-task/README.md`
- Modify: `skills/execute-task/metadata.yaml`

**Interfaces:**
- Consumes: Task 1-4 的脚本清单（目录说明要列全）
- Produces: 无；发布性收尾。

- [x] **Step 1: README 使用方式——chmod 句改通配**
...
```bash
cd "$REPO"
git add skills/execute-task/README.md skills/execute-task/metadata.yaml
