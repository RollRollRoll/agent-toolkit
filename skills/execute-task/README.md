...
"主 agent 自己在约定 seam 上红 → 绿 TDD → 验证 → 核证据 → 原子提交"的干净闭环，
进度用 checkbox + 账本记录可恢复，最后做一次整体五轴 + 测试质量 review + 覆盖核对回扫确认不落空，
收尾的合并 / PR 交用户拍板。
**任务内不派任何 subagent，也没有审查轮与修复轮**：闸门一核的是可机械判定的证据——红 → 绿输出当场落盘且齐、
收尾完整跑一次验证命令绿、HEAD 未漂移；架构级审查与修复集中到阶段 3 整体验收做一次，
那也是全流程唯一的 subagent 派发点。
...
Claude Code 用 Task / Agent 工具，Codex 用 agent thread（review 取只读 agent，fix 用内置 `worker`）。
两个平台共用同一套 TDD 闭环、证据核对和提交纪律。

## 目录说明

- `SKILL.md`：skill 主体（平台原生格式，含 frontmatter）。
- `references/orchestration.md`：调度与编排——依赖 / wave 排序、并行与 worktree 隔离、每任务的三份工作文件、进度账本与 safe-resume。
- `references/execution-loop.md`：每任务 TDD 闭环——seam 约定、好测试判据、三个反模式（实现耦合 / 同义反复 / 水平切片）、循环规则、验证节奏、闸门一证据核对、atomic commit、bug 诊断。
- `references/handoff-templates.md`：任务简报 / 执行记录的准备与格式，以及阶段 3 整体验收 review / fix 的派发 prompt。
...
- `scripts/workspace.sh`：工作目录 `.execute-task/` 的单一事实来源（建目录 + 自忽略 `.gitignore`，打印路径），其余脚本经它取目录。
