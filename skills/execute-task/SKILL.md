...
"代码写完了"不等于"做完了"。execute-task 把"做完"变成**有客观判据的事**：判据是**机制自带的**
...
**适用**：任务清单已确认（或手上有 split-task 的 tasks），要把一组拆好的开发任务推进到
...
5. 过门后先回写 checkbox（`- [ ]` → `- [x]`），确认提交仍在阶段 0 授权范围内，再把**代码 + 测试 + checkbox**
   ...
   `.execute-task/` ledger（起点 commit、本任务 commit、tdd 交回的结构疑虑），并用
...
2. 过**闸门二**：
   - **调用 review-changes**：派独立 subagent（**固定最强档**，显式指定模型，不继承当前档位）加载它，
     ...
     （五轴 + 测试质量）findings。
   - **覆盖核对回扫（本 skill 自己做，review-changes 不做）**：对回 split-task 的覆盖核对表、
...
> 2. **调用 tdd**：传简报路径、seam、验证方式 `pytest tests/test_models.py`、记录参数（`task-2` + `.execute-task/`）。
...
>    疑虑一条（"取数可拆两个 helper，按规则没动"）、记录路径 `.execute-task/task-2-record.md`。
