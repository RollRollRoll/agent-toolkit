...
**设计依据:** `skills/test-skill/docs/2026-07-02-test-skill-design.md`（下称"设计文档"）。执行每个任务前先读它。
...
**Files:**
- Create: `skills/test-skill/docs/2026-07-02-assumption-test-notes.md`
...
创建 `skills/test-skill/docs/2026-07-02-assumption-test-notes.md`，内容含：每条假设的实测命令、原始输出关键行、PASS/FAIL、对后续任务的影响；末尾给出**命令模板终稿**（首轮 + 续轮 + session_id 提取，逐字，供 Task 2 复制）。终稿初始形态（按实测修正）：
...
```bash
git -C /root/workspace/agent-toolkit add skills/test-skill/docs/2026-07-02-assumption-test-notes.md
...
2. 解析目标 skill 三种输入形式：仓库 id（`skills/<id>/SKILL.md`）/ SKILL.md 路径 / 已安装 skill 名（在插件缓存中定位）。定位失败即停并向用户列出找到的候选。
...
24. 存放：目标 skill 属于本仓库 → `skills/<被测id>/docs/test-report-YYYY-MM-DD.md`；否则 → 当前工作目录 `skill-test-reports/`。
...
```bash
git -C /root/workspace/agent-toolkit add skills/test-skill/SKILL.md
...
**Files:**
- Create: `skills/test-skill/references/judging-criteria.md`
- Create: `skills/test-skill/references/report-template.md`
- Create: `skills/test-skill/references/persona-template.md`
...
```bash
git -C /root/workspace/agent-toolkit add skills/test-skill/references/
...
**Files:**
- Create: `skills/test-skill/README.md`
- Create: `skills/test-skill/metadata.yaml`
...
```bash
git -C /root/workspace/agent-toolkit add skills/test-skill/README.md skills/test-skill/metadata.yaml
...
- Modify（仅当发现缺陷）: `skills/test-skill/SKILL.md` 及 references
...
编排者严格按 `skills/test-skill/SKILL.md` 从阶段 0 走到阶段 6（目标 skill 输入形式：SKILL.md 路径；报告落沙箱内 `skill-test-reports/`——验证第三方分支的存放规则）。
...
```bash
git -C /root/workspace/agent-toolkit add skills/test-skill/
...
**Files:**
- Create: `skills/refine-idea/docs/test-report-2026-07-02.md`（test-skill 按存放规则自动落此处）
- Modify（仅当发现缺陷）: `skills/test-skill/` 下文件
...
```bash
git -C /root/workspace/agent-toolkit add skills/refine-idea/docs/test-report-2026-07-02.md skills/test-skill/
...
- Modify: `skills/test-skill/metadata.yaml`（status: draft → active；updated_at 更新为发布日）
...
```bash
git -C /root/workspace/agent-toolkit add .claude-plugin/ skills/test-skill/metadata.yaml
