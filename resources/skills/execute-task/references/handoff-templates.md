...
4. **任务基线** `[BASE_FILE]`：在派执行 subagent 前运行本 skill 目录下的
   `scripts/task-baseline.sh <任务编号>`。它要求真实工作区没有暂存、未暂存或未跟踪改动，并记录当前 HEAD；
   失败就先处理现有改动，**不要覆盖基线或带脏开工**。同一任务 fix / 复审期间不要重跑基线。
5. **diff 文件**在派 review 前生成：运行本 skill 目录下的 `scripts/review-diff.sh <任务编号>`——
   它会校验 HEAD 仍是任务基线，按轮次自动递增命名（R1/R2/R3…），收集相对 HEAD 的暂存、未暂存、删除、
   未跟踪及 binary 变化，以 `-U10` 扩展上下文，且不修改真实 index，
...
diff 文件：[DIFF_FILE]（相对任务基线 HEAD 的完整未提交改动，含暂存 / 未暂存 / 删除 / 未跟踪 / binary 与扩展上下文）
...
- 派执行前 `task-baseline.sh` 确认了真实工作区干净？派 review 时 `review-diff.sh` 校验了 HEAD 未漂移并包含所有未提交变化？
