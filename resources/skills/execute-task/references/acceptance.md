...
- **整体验收通过后，调用 `finish-branch` 做收尾**：它会验证测试 → 检测 repo/worktree 状态 →
  给出合并到主干 / 本地保留 / 创建 PR / 丢弃选项 → 清理 worktree / 临时分支。
- **危险操作 gate**：合并 / PR / push / 丢弃 **不自动做**，由 finish-branch 交用户明确确认（呼应 CLAUDE.md）。
