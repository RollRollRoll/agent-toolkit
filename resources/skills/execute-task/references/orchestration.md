...
- **何时用**：多个任务并行、且可能改同一区域代码、想彻底隔离时——每个 subagent 在独立 git worktree 工作，互不干扰。
- **怎么用**：每个并行任务一个 worktree，完成后各自 atomic commit，再按 wave 合并回来。
- **可选**：单任务串行、或并行任务文件不交叠时**不需要** worktree——别为隔离而隔离，徒增复杂度。
