...
2. **每任务走干净闭环** —— 执行 subagent（TDD → 验证）→ 独立 review subagent 审查 → 独立 fix subagent 修复 → 原子提交，不绿不算完成。
...
- **atomic commit**：复审绿、过闸门一后才提交；一个任务一个原子提交，提交信息可追溯到任务。
...
1. 对每个任务，按「核心一」闭环**分别派发独立 subagent**：先派执行 subagent（TDD → 验证，不 commit），
   再派 review subagent 独立审查；有 Critical / Important 再派 fix subagent 修复并复审
   （修复轮次超限仍不过 → 停下交用户判断）。
2. 过「闸门一」（验收标准 + 验证方式绿、复审通过）后才提交、才回写 checkbox + ledger。
3. 同 wave 任务可并行执行；**wave 间串行**（前一波合并 / 稳定后再下一波）。
4. 每 2~3 个任务到 checkpoint 跑一次相关测试 / 构建，确认到这里是稳的。

### 阶段 3 · 整体验收

1. 过「闸门二」：**派独立 review subagent** 做五轴 review + 覆盖核对回扫，跑全套测试 / 构建 / typecheck 绿。
2. 不绿 / 有落空 → **派独立 fix subagent** 修复（或回阶段 2 补任务），修完再验收。
...
> 1. **派执行 subagent** 接 T2「`list_experiments()` 查询 + 校验 + 稳定排序」：
...
> 2. **另派 review subagent**（fresh，只带 T2 验收标准 + diff）：对照验收标准逐条核——四条都满足；
>    纯函数无副作用、可单测 ✓。若审出 Critical / Important（如排序不稳定），**再派 fix subagent** 带着 review 发现修复 →
>    复审（超 3 轮仍有 Critical / Important 就停下问用户）；Minor 记录不阻塞。
> 3. 复审绿、过**闸门一** → 主 agent `git commit -m "feat: list_experiments 查询+校验+稳定排序"`（atomic）。
> 4. **回写账本**：tasks.md 的 T2 改 `- [x]`，ledger 记 commit。
>
