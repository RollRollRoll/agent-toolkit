...
2. **定并行与隔离**：同 wave、不冲突文件的任务可并行；并行且可能改同区域时，用 **worktree 隔离**（可选，按需）。
...
1. **给出收尾选项交用户拍板**：合并到主干 / 提 PR / 留在分支——**合并 / PR / push 不自动做**（危险操作 gate）。
2. **清理**：删临时分支 / worktree、清理遗留的调试代码与打印。
3. execute-task 到"整体验收通过 + 收尾决策交付"为止。
...
- **收尾不自动合并**：执行完给出合并 / PR / 留分支选项，由用户拍板。
...
9. **自包含** —— 不在运行时依赖外部插件；subagent 不可用降级为同一套纪律的顺序执行。
